...
receive_chars(struct uart_xr_port *up, unsigned int *status)
{
	...
	if (unlikely(lsr & (UART_LSR_BI | UART_LSR_PE | UART_LSR_FE | UART_LSR_OE))) 
	{
		...
		*/
		...
		{
		      printk("handling Over Run error....\n");	
		}
		else if (lsr & UART_LSR_PE)
		{
			flag = TTY_PARITY;
			printk("handling UART_LSR_PE Error....\n");	
...
//    printk("Receive %d Bytes\n",datasize_in_fifo);  
	...
	for(i = 0; i < datasize_in_fifo; i++)
	{
		...
		{
		       printk("uart_handle_sysrq_char failed\n");
			continue;
		}
...
static void transmit_chars(struct uart_xr_port *up)
{
	...
	{	
		...
		{			
			...
			pSent = (unsigned char *)&(xmit->buf[xmit->tail]);
			for(i=0;i<tmp;i++)
			{
			  //printk("1 Send Bytes:0x%02x\n",pSent[i]);
			  serial_out(up,XR_17v35x_UART_RHR,pSent[i]);
			}   
...
static irqreturn_t serialxr_interrupt(int irq, void *dev_id)
{
	...
	do {
		...
		if (!(iir & UART_IIR_NO_INT)) {
			printk(KERN_ERR "aitech: We are here...\n");
			//serialxr_handle_port(up);
			//handled = 1;
			//end = NULL;
		} else if (end == NULL)
...
static int serialxr_startup(struct uart_port *port)
{
	...
	{
		serial_out(up, XR_17V35X_EXTENDED_RXTRG, 4); // 25x
		serial_out(up, XR_17V35X_EXTENDED_TXTRG, 32);
	}
...
static int __init serialxr_init(void)
{
	...
	printk(KERN_INFO "Exar PCIe (XR17V35x) serial driver Revision: 2.3\n");
...
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("Exar PCIe specific serial driver for XR17V35x- Revision: 2.3");
